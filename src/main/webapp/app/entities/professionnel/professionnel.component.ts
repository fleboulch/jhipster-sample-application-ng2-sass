...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/professionnel'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Professionnel) {
        return item.id;
    }
    registerChangeInProfessionnels() {
        this.eventSubscriber = this.eventManager.subscribe('professionnelListModification', (response) => this.loadAll());
    }
