...
@Component({
    selector: 'jhi-partenariat',
    templateUrl: './partenariat.component.html'
})
export class PartenariatComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/partenariat'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/partenariat', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: Partenariat) {
        return item.id;
    }
    registerChangeInPartenariats() {
        this.eventSubscriber = this.eventManager.subscribe('partenariatListModification', (response) => this.loadAll());
    }
