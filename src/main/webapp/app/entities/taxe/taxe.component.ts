...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/taxe'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Taxe) {
        return item.id;
    }
    registerChangeInTaxes() {
        this.eventSubscriber = this.eventManager.subscribe('taxeListModification', (response) => this.loadAll());
    }
